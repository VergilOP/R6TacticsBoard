...
- 资源管理页的地图列表保持文字显示，不显示地图缩略图，避免把整张地图资源误当作小图标使用。
- 资源管理页的干员列表显示干员小图标，用于提升资源辨识度。
- 数量配置页的通用道具表显示道具小图标，技能道具表显示干员小图标与技能小图标。
...
- 页面内的小图标统一通过 [asset_icons.py](../../widgets/asset_icons.py) 生成，避免重复裁剪透明边距或引入无缓存的图片处理逻辑。
