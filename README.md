...
- 继续做低风险结构整理，下一步优先拆分 `map_scene.py` 的 overlay / token / operator 职责
...
- 完成编辑器主控低风险结构整理，将道具/技能、属性面板、时间轴、播放、视图、工程状态和画布同步拆分为独立 helper
- 拆分资源索引层，将地图、干员、通用道具、资源模型和辅助函数从 `asset_registry.py` 中分离
- 保留 `asset_registry.py` 作为对外兼容门面，降低全项目 import 变更风险
- 同步更新代码地图、编辑器工作流和各级 README，明确当前功能入口、资源语义和后续结构维护方向
- 补齐 README 链接、路径和文本格式检查，保证文档适合 GitHub 与后续 LLM 辅助阅读

### v0.6.4

- 修复一次性通用道具与一次性技能道具在时间轴后续关键帧中的继承逻辑，改为仅继承已使用数量、不继承地图图标
- 统一保留型与一次性道具/技能的显示规则，保留型继续显示在地图上，一次性仅在当前显式放置帧显示
- 新增右侧属性面板的当前帧清空按钮，可分别清空当前帧的通用道具与技能道具部署
- 优化通用道具与技能道具数量配置页，补齐保留/一次性配置链路并统一到资源定义与编辑器显示逻辑
- 修复关键帧切换时因重复读取资源索引导致的明显卡顿问题，补充资源缓存并按修改自动失效

### v0.6.3

- 新增软墙 / `Hatch` 面战术状态编辑，支持加固、过人洞、蹲姿过人洞、翻越洞、脚洞和对枪洞规则约束
- 将 `hatch` 互动点语义迁移到 `Hatch` 面，并兼容旧版地图资源自动迁移
- 重构楼梯为起点 / 终点 / 折线轨迹模型，Debug 页支持折线绘制、右键完成和方向吸附
- 优化 `2.5D` 中的楼梯跨层表现，按楼梯折线长度自适应分配上下楼轨迹
- 整理编辑页右侧属性区，拆分为 `干员 / 装修 / 关键帧` 页签并补充页签记忆
- 优化当前上下文提示、关键帧列头选择反馈和危险操作按钮层级
- 调整干员显示控制，支持独立隐藏图标 / 名字，并增加整体干员大小滑块
- 明确控件可用性分层：可操作项正常显示，上下文不可用项灰掉，无意义项隐藏

### v0.6.2

- 将原有 `hatch` 互动点功能迁移到 `Hatch` 面，仅保留 `Hatch` 面承担跨层联通语义
- 兼容旧版地图资源，旧 `hatch` 互动点加载时会自动迁移为 `Hatch` 面
- 重构楼梯互动点表现，升级为带起点、终点与折线轨迹的楼梯对象
- 地图 Debug 页新增楼梯折线绘制流程，支持连续落点、右键完成与水平 / 竖直 / `45°` 自动校准
- 编辑页跨层寻路与播放改为区分楼梯入口点和出口点，跨层移动表现更准确
- 修复 `Hatch` 面加固状态与跨层联通迁移过程中的若干兼容性问题
