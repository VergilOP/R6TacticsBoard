...
- 继续完善 `2.5D 总览`，补齐 `stairs / hatch` 专用图元、跨层连线和路径高亮
- 校准各地图的 `overview_2_5d` 参数，减少楼层贴图的高度、中心和默认视角偏差
- 完成 `2.5D` 模式下的干员图标、选中反馈和遮挡表达细化
- 为电竞数据接入外部 API 预留统一数据源和字段映射层
- 收口工程级资源管理、时间轴补强和发布流程
